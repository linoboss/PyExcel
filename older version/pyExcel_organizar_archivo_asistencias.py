...
class Reorganizar:
    def __init__(self):
        #Solicitar al usuario la eleccion del archivo
...
def prueba3():
    ...
    header = [['', '', '', 'Horario', '', '', '', '', '', ''],
              ['', 'Matutino', '', 'Vespertino', '', 'Nocturno', '', '', '', ''],
              ['Nombre', 'entrada', 'salida', 'entrada', 'salida', 'entrada', 'salida', 'Horas laboradas',
               'Tiempo extra', 'Tiempo ausente']]
                ...
                for day in week:
                    if day != 0:
                        ...
                        try:
                            ...
                            to_excel.append(texto)
                            to_excel.append(header)
                            ...
                            for worker, schedule in workday.get_workers_info().items():
                                ...
                                #Recorre los 6 valores de  horary y les asigna rojo si el trabajador no marcó
                                # o verde si marco bien.
                                excel_index = to_excel.index()
                                hora_marcada = ['B', 'C', 'D', 'E', 'F', 'G']
                                for j in range(len(horary)):
                                    if horary[j] == '0:00:00':
                                        Range(hora_marcada[j] + str(excel_index)).color = (255, 150, 150)

                                    else:
                                        Range(hora_marcada[j] + str(excel_index)).color = (150, 255, 150)

                                to_excel.append(line_to_excel)
                            to_excel.skip_line()
                #Muestreo del resumen semanal
                to_excel.append('Resumen Semanal', column='K')
                to_excel.append(['Nombre', 'Tiempo trabajado', 'Tiempo extra', 'Tiempo ausente'], column='K')
                resumen.append(['Resumen Semanal'])
                resumen.append(['Nombre', 'Tiempo trabajado', 'Tiempo extra', 'Tiempo ausente'])

                for w, info in week_resume.get_info().items():
                    to_excel.append([w, info['tiempo trabajado'], info['tiempo extra'],
                                                 info['tiempo ausente']], column='K')
                    resumen.append([w, info['tiempo trabajado'], info['tiempo extra'],
                                            info['tiempo ausente']])
                resumen.append([])
                to_excel.skip_line()
            #Muestreo del resumen mensual
            to_excel.append('Resumen Mensual', column='K')
            to_excel.append(['Nombre', 'Tiempo trabajado', 'Tiempo extra', 'Tiempo ausente'], column='K')
            resumen.append(['Resumen Mensual'])
            resumen.append(['Nombre', 'Tiempo trabajado', 'Tiempo extra', 'Tiempo ausente'])
            for w, info in month_resume.get_info().items():
                to_excel.append([w, info['tiempo trabajado'], info['tiempo extra'], info['tiempo ausente']], column='K')
                resumen.append([w, info['tiempo trabajado'], info['tiempo extra'], info['tiempo ausente']])
            resumen.append([])
    to_excel.autofit()
    to_excel.add_sheet('Resumen')
    to_excel.append(resumen)
    to_excel.autofit()
